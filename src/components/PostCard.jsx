...
export default function PostCard({ post, onExpand, onVote }) {
  const typeInfo = POST_TYPES[post.type]

  return (
    <article
      onClick={() => onExpand(post)}
      className="bg-white rounded-2xl shadow-sm border border-border p-4 cursor-pointer hover:shadow-md transition-shadow active:scale-[0.99] transition-transform"
    >
      {/* Top row */}
